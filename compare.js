...
// ---- Deep link: read a,b from URL ----
function findByQuery(q){
  if (!q) return null;
  const list = ALL();
  const needle = decodeURIComponent(q).trim().toLowerCase();
  ...
  // 2) exact name
  const exact = list.find(p => (p.name||'').trim().toLowerCase() === needle);
  if (exact) return exact.id;

  // 3) startsWith/contains
  const starts = list.find(p => (p.name||'').trim().toLowerCase().startsWith(needle));
  if (starts) return starts.id;

  const contains = list.find(p => (p.name||'').toLowerCase().includes(needle));
  if (contains) return contains.id;

  // 4) numeric index fallback: f<number>
  if (/^\d+$/.test(needle)) {
...
function applyDeepLink(){
  ...
  const aId = findByQuery(a);
  const bId = findByQuery(b);

  if (aId) leftSel.value = aId;
  if (bId) rightSel.value = bId;
}

// ---- Share link ----
function updateShareLink(pushToAddressBar=false){
  const u = new URL(window.location.href);
  u.searchParams.set('a', leftSel.value);
  u.searchParams.set('b', rightSel.value);

  const val = u.toString();
...
$('#copyShare')?.addEventListener('click', async ()=>{
  ...
  try{
    ...
    // minimalist feedback (be alertless if desired)
    const btn = $('#copyShare');
...
$('#clearBtn')?.addEventListener('click', ()=>{
  fillSelectors(); applyDeepLink(); render();
});
