(function () {
  ...
  function isContactLike(strRaw) {
    ...
    if (str.includes("http://") || str.includes("https://") || str.includes("www."))
      return true;
    if (str.includes("telegram") || str.includes("tg:") || str.includes("whatsapp"))
      return true;
  ...
  function hasAnyCoreData(soul) {
    ...
    if (
      cleanList(soul.hobbies || soul.passions || soul.interests || []).length
    )
      return true;
  ...
  function chineseZodiacCompatibility(baseChineseRaw, matchChineseRaw) {
    ...
    return {
      ...
      label:
        "Different Chinese signs — diversity that can enrich the dynamic.",
    };
  }
  ...
  function renderValuesSection(baseSoul, match, breakdown) {
    ...
    if (ui.valuesSummary) {
      ui.valuesSummary.textContent = buildValuesSummary(
        baseSoul,
        match,
        breakdown
      );
  ...
  function renderAstroSection(baseSoul, match, breakdown) {
    ...
    if (ui.astroSummary) {
      ui.astroSummary.textContent = buildAstroSummary(
        baseSoul,
        match,
        breakdown
      );
  ...
  function renderStepSection(baseSoul, match, breakdown) {
    ...
    if (ui.stepHighlight) {
      ui.stepHighlight.textContent = buildStepHighlight(
        baseSoul,
        match,
        breakdown
      );
